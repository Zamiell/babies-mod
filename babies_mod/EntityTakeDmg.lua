...
-- D Baby
EntityTakeDmg.functions[101] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
  ...
  if (
    damageSource.Type == EntityType.ENTITY_EFFECT -- 1000
    and damageSource.Variant == EffectVariant.PLAYER_CREEP_RED -- 46
  ) then
...
-- Fang Demon Baby
EntityTakeDmg.functions[281] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
  ...
  if (
    damageSource.Type == EntityType.ENTITY_EFFECT -- 1000
    and damageSource.Variant == EffectVariant.CRACK_THE_SKY -- 19
  ) then
...
-- Rider Baby
EntityTakeDmg.functions[295] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
  ...
  if (
    damageSource.Type == EntityType.ENTITY_PLAYER -- 1
    and damageSource.Variant == 0
  ) then
...
-- Boxers Baby
EntityTakeDmg.functions[337] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
  ...
  if (
    damageSource.Type == EntityType.ENTITY_TEAR -- 2
    and damageSource.Entity.SubType == 1
  ) then
...
-- Elf Baby
EntityTakeDmg.functions[377] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
  ...
  if (
    damageSource.Type == EntityType.ENTITY_EFFECT -- 1000
    and damageSource.Variant == EffectVariant.SPEAR_OF_DESTINY -- 83
  ) then
    local damage = g.p.Damage * 4
    g.run.dealingExtraDamage = true
    entity:TakeDamage(damage, 0, EntityRef(g.p), damageCountdownFrames)
...
-- Astronaut Baby
EntityTakeDmg.functions[406] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
...
-- Road Kill Baby
EntityTakeDmg.functions[507] = function(
  entity,
  damageAmount,
  damageFlag,
  damageSource,
  ...
  if (
    ...
    and damageSource.Variant == FamiliarVariant.POINTY_RIB -- 127
  ) then
