...
PostProjectileUpdate.functions[61] = function(projectile)
  if (
    ...
    and projectile.Parent:HasEntityFlags(EntityFlag.FLAG_FRIENDLY) -- 1 << 29
  ) then
...
PostProjectileUpdate.functions[109] = function(projectile)
  ...
  if (
    ...
    and projectile.SpawnerType ~= EntityType.ENTITY_ISAAC -- 102
  ) then
...
PostProjectileUpdate.functions[224] = function(projectile)
  ...
  if (
    data.spedUp == nil
    and projectile.SpawnerType ~= EntityType.ENTITY_MOMS_HEART -- 78
    and projectile.SpawnerType ~= EntityType.ENTITY_ISAAC -- 102
  ) then
    data.spedUp = true
...
PostProjectileUpdate.functions[318] = function(projectile)
  ...
  -- (this cannot be done in the MC_POST_PROJECTILE_INIT callback since "projectile.SpawnerType" is
  -- empty)
  if (
    projectile.FrameCount == 1
    and projectile.SpawnerType == EntityType.ENTITY_FIREPLACE -- 33
  ) then
    projectile:Remove()
  end
end
