...
// Lil' Baby
functionMap.set(36, (pickup: EntityPickup) => {
  // Everything is tiny
  pickup.SpriteScale = Vector(0.5, 0.5);
});

// Big Baby
functionMap.set(37, (pickup: EntityPickup) => {
  // Everything is giant
...
// Shopkeeper Baby
functionMap.set(215, (pickup: EntityPickup) => {
  // Free items in shops
...
// Wizard Baby
functionMap.set(253, (pickup: EntityPickup) => {
...
// Demon Baby
functionMap.set(527, (pickup: EntityPickup) => {
  // Free devil deals
  if (pickup.Variant === PickupVariant.PICKUP_COLLECTIBLE) {
    const roomType = g.r.GetType();
    if (
