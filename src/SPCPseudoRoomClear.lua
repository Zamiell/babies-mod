local SPCPseudoRoomClear = {}

-- Includes
local SPCGlobals = require("src/spcglobals")

function SPCPseudoRoomClear:PostUpdate()
  -- Local variables
  local game = Game()
  ...
  if roomFrameCount == 1 and
    ...
    SPCPseudoRoomClear:InitializeDoors()
    return
  end

  SPCPseudoRoomClear:CheckPseudoClear()
end

function SPCPseudoRoomClear:InitializeDoors()
  -- Local variables
  local game = Game()
  local room = game:GetRoom()
  ...
  room:SetClear(true)
  SPCGlobals.run.roomPseudoClear = false
  for i = 0, 7 do
    ...
    if door ~= nil and
      ...
      SPCGlobals.run.roomDoorsModified[#SPCGlobals.run.roomDoorsModified + 1] = i
...
function SPCPseudoRoomClear:CheckPseudoClear()
  -- Local variables
  local game = Game()
  local gameFrameCount = game:GetFrameCount()

  -- Don't do anything if the room is already cleared
  if SPCGlobals.run.roomPseudoClear then
    return
  end

  -- If a frame has passed since an enemy died, reset the delay counter
  if SPCGlobals.run.roomClearDelayFrame ~= 0 and
     gameFrameCount >= SPCGlobals.run.roomClearDelayFrame then
  ...
  for i, entity in pairs(Isaac.GetRoomEntities()) do
    ...
    if npc ~= nil and
       ...
       SPCPseudoRoomClear:AttachedNPC(npc) == false then
  ...
  if aliveEnemiesCount == 0 and
     SPCGlobals.run.roomClearDelayFrame == 0 and
     SPCPseudoRoomClear:CheckAllPressurePlatesPushed() and
     gameFrameCount > 1 then -- To prevent misc. bugs

    SPCPseudoRoomClear:ClearRoom()
  end
end

-- This code is copied from Racing+
function SPCPseudoRoomClear:AttachedNPC(npc)
...
function SPCPseudoRoomClear:CheckAllPressurePlatesPushed()
  -- Local variables
  local game = Game()
  local room = game:GetRoom()
  ...
  if room:HasTriggerPressurePlates() == false or
     SPCGlobals.run.roomButtonsPushed then
  ...
  SPCGlobals.run.roomButtonsPushed = true
  return true
end

-- This roughly emulates what happens when you normally clear a room
function SPCPseudoRoomClear:ClearRoom()
  -- Local variables
  local game = Game()
  local room = game:GetRoom()
  ...
  for i = 1, #SPCGlobals.run.roomDoorsModified do
    local doorNum = SPCGlobals.run.roomDoorsModified[i]
