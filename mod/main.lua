...
function ____exports.addCharge(self, singleCharge)
    ...
    local activeItemMaxCharges = ____exports.getItemMaxCharges(nil, activeItem)
    ...
    if roomShape >= 8 then
        chargesToAdd = 2
    elseif g.p:HasTrinket(TrinketType.TRINKET_AAA_BATTERY) and (activeCharge == (activeItemMaxCharges - 2)) then
        chargesToAdd = 2
    elseif ((g.p:HasTrinket(TrinketType.TRINKET_AAA_BATTERY) and (activeCharge == activeItemMaxCharges)) and g.p:HasCollectible(CollectibleType.COLLECTIBLE_BATTERY)) and (batteryCharge == (activeItemMaxCharges - 2)) then
        chargesToAdd = 2
    end
...
function ____exports.getScreenCenterPosition(self)
        ...
        while i < g.g:GetNumPlayers() do
            do
                ...
                if player:NeedsCharge() then
                    ...
                    if shape >= 8 then
                        chargesToAdd = 2
                    elseif player:HasTrinket(TrinketType.TRINKET_AAA_BATTERY) and (activeCharge == (activeItemMaxCharges - 2)) then
                        chargesToAdd = 2
                    elseif ((player:HasTrinket(TrinketType.TRINKET_AAA_BATTERY) and (activeCharge == activeItemMaxCharges)) and player:HasCollectible(CollectibleType.COLLECTIBLE_BATTERY)) and (batteryCharge == (activeItemMaxCharges - 2)) then
                        chargesToAdd = 2
                    end
...
functionMap:set(
    ...
    function()
            ...
            while i <= baby.num do
                while true do
                    do
                        ...
                        if __TS__ArrayIncludes(randomFloorIndexes, randomFloorIndex) then
                            goto __continue63
                        end
                        if randomFloorIndex == startingRoomIndex then
                            goto __continue63
                        end
                        __TS__ArrayPush(randomFloorIndexes, randomFloorIndex)
                        break
                    end
                    ::__continue63::
                end
        ...
        for ____, roomIndex in ipairs(randomFloorIndexes) do
            g.l.LeaveDoor = -1
